...
data ServantDeps r m = ServantDeps 
  { _subscribeToState :: m (r, Stream (Of r) IO ())
...
serveLiveViewServant liveView servDeps = initialRenderEndpoint :<|> liveRenderEndpoint
  where
    initialRenderEndpoint = do
      initialState <- fst <$> _subscribeToState servDeps
      pure $ _html $ getLiveViewResult initialState liveView
    liveRenderEndpoint conn = do
      (initS, states) <- _subscribeToState servDeps
      inpChan <- liftIO STM.newTChanIO
      let actionThread = forever $ do
            (receivedMsg :: T.Text) <- WS.receiveData conn
            Prelude.putStrLn $ show receivedMsg
            case (decodeStrict
                  $ encodeUtf8 receivedMsg :: Maybe (ActionCall, Clock)) of
              Nothing -> pure ()
          ...
          stateThread = S.mapM_ (STM.atomically . STM.writeTChan inpChan . InputState) states
                -- _actionCallback servDeps actionCall
      ...
      flip S.mapM_ (_outputStream outputs) $ \outEntry -> case outEntry of
          OutputAction call -> _actionCallback servDeps call
          OutputPatch (patchList, clock) -> liftIO $ WS.sendTextData conn $ encode (("patch", patchList), clock)
